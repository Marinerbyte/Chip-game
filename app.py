import os
import json
import time
import threading
import io
import requests
import websocket
from flask import Flask, render_template_string, request, jsonify, send_file
from PIL import Image, ImageDraw, ImageFont

app = Flask(__name__)

# --- GLOBAL VARIABLES (STATE STORAGE) ---
BOT_STATE = {
    "ws": None,
    "connected": False,
    "user": "",
    "pass": "",
    "room": "",
    "thread": None,
    "domain": ""
}

GAME_STATE = {
    "active": False,
    "player": None,
    "bombs": [],
    "eaten": [],
    "user_avatars": {} 
}

LOGS = []

def add_log(msg, type="sys"):
    timestamp = time.strftime("%H:%M:%S")
    LOGS.append({"time": timestamp, "msg": msg, "type": type})
    if len(LOGS) > 100: LOGS.pop(0)

# --- WEBSOCKET HANDLERS (SERVER SIDE) ---

def on_message(ws, message):
    try:
        data = json.loads(message)
        if data.get("handler") == "receipt_ack": return

        # Capture Avatar
        if data.get("from") and data.get("avatar_url"):
            GAME_STATE["user_avatars"][data["from"]] = data["avatar_url"]

        # Log & Game Logic
        if data.get("handler") == "room_event" and data.get("type") == "text":
            add_log(f"[{data['from']}]: {data['body']}", "in")
            process_game_logic(data['from'], data['body'])
            
        elif data.get("handler") == "login_event":
            if data["type"] == "success":
                add_log("Login Success. Joining Room...", "sys")
                join_pkt = {"handler": "room_join", "id": str(time.time()), "name": BOT_STATE["room"]}
                ws.send(json.dumps(join_pkt))
            else:
                add_log(f"Login Failed: {data.get('reason')}", "err")
                BOT_STATE["connected"] = False

    except Exception as e:
        print(f"Error: {e}")

def on_error(ws, error):
    add_log(f"Socket Error: {error}", "err")

def on_close(ws, close_status_code, close_msg):
    add_log("Disconnected from Server.", "err")
    BOT_STATE["connected"] = False

def on_open(ws):
    BOT_STATE["connected"] = True
    add_log("Socket Connected. Authenticating...", "sys")
    login_pkt = {
        "handler": "login",
        "id": str(time.time()),
        "username": BOT_STATE["user"],
        "password": BOT_STATE["pass"],
        "platform": "web"
    }
    ws.send(json.dumps(login_pkt))
    
    # Keep-Alive Thread
    def run_ping():
        while BOT_STATE["connected"]:
            time.sleep(20)
            try:
                if BOT_STATE["ws"]:
                    BOT_STATE["ws"].send(json.dumps({"handler": "ping"}))
            except:
                break
    threading.Thread(target=run_ping, daemon=True).start()

# --- HELPER: SEND MESSAGE ---
def send_room_msg(text, msg_type="text", url=""):
    if BOT_STATE["ws"] and BOT_STATE["connected"]:
        pkt = {
            "handler": "room_message",
            "id": str(time.time()),
            "room": BOT_STATE["room"],
            "type": msg_type,
            "body": text,
            "url": url,
            "length": "0"
        }
        try:
            BOT_STATE["ws"].send(json.dumps(pkt))
            if msg_type == "text":
                add_log(f"BOT >> {text.splitlines()[0]}...", "out")
            else:
                add_log(f"BOT >> SENT IMAGE", "out")
        except Exception as e:
            add_log(f"Send Error: {e}", "err")

# --- GAME LOGIC (UNCHANGED - 4 Chips Rule) ---
def process_game_logic(user, msg):
    msg = msg.strip().lower()
    if user.lower() == BOT_STATE["user"].lower(): return

    # !START
    if msg == "!start":
        if GAME_STATE["active"]:
            send_room_msg(f"⚠ Game running! {GAME_STATE['player']} is playing.")
            return
        
        import random
        GAME_STATE["active"] = True
        GAME_STATE["player"] = user
        GAME_STATE["eaten"] = []
        GAME_STATE["bombs"] = random.sample(range(1, 10), 2)
        
        add_log(f"Game Started by {user}. Bombs: {GAME_STATE['bombs']}", "game")
        grid = render_grid()
        send_room_msg(f"🎮 START! Player: {user}\nAvoid 2 Bombs! Eat 4 Chips to WIN.\nType !eat <number>\n\n{grid}")

    # !EAT
    elif msg.startswith("!eat "):
        if not GAME_STATE["active"]: return
        if user != GAME_STATE["player"]: return
        
        try: num = int(msg.split()[1])
        except: return send_room_msg("⚠ Invalid number.")

        if num < 1 or num > 9: return send_room_msg("⚠ Choose 1-9.")
        if num in GAME_STATE["eaten"]: return send_room_msg("⚠ Already eaten!")

        if num in GAME_STATE["bombs"]:
            GAME_STATE["active"] = False
            grid = render_grid(reveal=True, exploded=num)
            send_room_msg(f"💥 BOOM! You ate a BOMB at #{num}!\n💀 GAME OVER.\nBombs: {GAME_STATE['bombs']}\n\n{grid}")
            add_log(f"Game Over: {user} hit bomb.", "err")
        else:
            GAME_STATE["eaten"].append(num)
            # WIN CONDITION: 4 CHIPS
            if len(GAME_STATE["eaten"]) == 4:
                GAME_STATE["active"] = False
                grid = render_grid(reveal=True)
                send_room_msg(f"🎉 WINNER! {user} ate 4 chips!\n🥔 CHAMPION! Generating Prize...\n\n{grid}")
                
                time.sleep(1)
                avatar = GAME_STATE["user_avatars"].get(user, "")
                send_winner_image(user, avatar)
                add_log(f"Victory: {user}", "game")
            else:
                grid = render_grid()
                send_room_msg(f"🥔 SAFE! ({len(GAME_STATE['eaten'])}/4)\n{grid}")

def render_grid(reveal=False, exploded=None):
    icons = ["1️⃣","2️⃣","3️⃣","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣"]
    txt = ""
    for i in range(1, 10):
        if reveal and i == exploded: txt += "💥 "
        elif reveal and i in GAME_STATE["bombs"]: txt += "💣 "
        elif i in GAME_STATE["eaten"]: txt += "🥔 "
        else: txt += icons[i-1] + " "
        if i % 3 == 0 and i != 9: txt += "\n"
    return txt

def send_winner_image(username, avatar):
    domain = BOT_STATE.get("domain", "")
    if domain:
        img_url = f"{domain}winner-card?name={username}&avatar={requests.utils.quote(avatar)}"
        send_room_msg("", msg_type="image", url=img_url)
    else:
        send_room_msg("🏆 [Winner Image Failed - Domain Error] 🏆")

# --- FLASK ROUTES ---

@app.route('/')
def index():
    return render_template_string(HTML_PAGE, connected=BOT_STATE["connected"])

@app.route('/connect', methods=['POST'])
def start_bot():
    data = request.json
    if BOT_STATE["connected"]: return jsonify({"status": "Already Online"})

    BOT_STATE["user"] = data["u"]
    BOT_STATE["pass"] = data["p"]
    BOT_STATE["room"] = data["r"]
    BOT_STATE["domain"] = request.url_root 

    def run_ws():
        websocket.enableTrace(False)
        ws = websocket.WebSocketApp("wss://chatp.net:5333/server",
                                  on_open=on_open, on_message=on_message, on_error=on_error, on_close=on_close)
        BOT_STATE["ws"] = ws
        ws.run_forever()

    t = threading.Thread(target=run_ws)
    t.start()
    BOT_STATE["thread"] = t
    return jsonify({"status": "Initializing System..."})

@app.route('/disconnect', methods=['POST'])
def stop_bot():
    if BOT_STATE["ws"]:
        BOT_STATE["ws"].close()
        BOT_STATE["ws"] = None
    BOT_STATE["connected"] = False
    GAME_STATE["active"] = False
    add_log("System Manual Shutdown.", "err")
    return jsonify({"status": "System Halted."})

@app.route('/logs')
def get_logs():
    return jsonify({"logs": LOGS, "connected": BOT_STATE["connected"]})

@app.route('/winner-card')
def winner_card():
    try:
        username = request.args.get('name', 'Winner')
        avatar_url = request.args.get('avatar', '')
        width, height = 600, 300
        img = Image.new('RGB', (width, height), color=(20, 20, 20))
        draw = ImageDraw.Draw(img)
        draw.rectangle([0, 0, width-1, height-1], outline="#00f3ff", width=5)
        draw.rectangle([10, 10, width-10, height-10], outline="#ffd700", width=2)
        try:
            if avatar_url and avatar_url != "undefined":
                headers = {'User-Agent': 'Mozilla/5.0'}
                response = requests.get(avatar_url, headers=headers, timeout=5)
                avi = Image.open(io.BytesIO(response.content)).convert("RGBA")
                avi = avi.resize((150, 150))
                mask = Image.new("L", (150, 150), 0)
                ImageDraw.Draw(mask).ellipse((0, 0, 150, 150), fill=255)
                img.paste(avi, (50, 75), mask)
            else:
                draw.ellipse((50, 75, 200, 225), fill="#444", outline="#fff")
        except:
            draw.ellipse((50, 75, 200, 225), fill="#444", outline="#fff")

        draw.text((230, 80), "--- WINNER ---", fill="#ffd700") 
        draw.text((230, 110), f"Name: {username}", fill="#ffffff")
        draw.text((230, 140), "Result: 4 Safe Chips", fill="#00ff41")
        draw.text((230, 170), "Status: TITAN CHAMPION", fill="#00f3ff")
        
        img_io = io.BytesIO()
        img.save(img_io, 'PNG')
        img_io.seek(0)
        return send_file(img_io, mimetype='image/png')
    except Exception as e:
        return str(e), 500

# --- ENHANCED UI ---
HTML_PAGE = """
<!DOCTYPE html>
<html>
<head>
    <title>TITAN OS - SERVER CONTROL</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Share+Tech+Mono&display=swap" rel="stylesheet">
    <style>
        :root { --neon: #00f3ff; --term: #00ff41; --danger: #ff003c; --bg: #020202; --panel: #0a0a0a; }
        body { background: var(--bg); color: var(--neon); font-family: 'Share Tech Mono', monospace; margin: 0; display: flex; flex-direction: column; height: 100vh; overflow: hidden; }
        
        /* HEADER */
        .header { padding: 15px; background: rgba(0,0,0,0.8); border-bottom: 2px solid var(--neon); display: flex; justify-content: space-between; align-items: center; box-shadow: 0 0 20px rgba(0, 243, 255, 0.2); }
        .title { font-size: 18px; letter-spacing: 2px; font-weight: bold; text-shadow: 0 0 5px var(--neon); }
        .status-badge { padding: 5px 10px; border: 1px solid #333; border-radius: 4px; font-size: 12px; }
        .online { color: var(--bg); background: var(--term); box-shadow: 0 0 10px var(--term); }
        .offline { color: var(--bg); background: var(--danger); box-shadow: 0 0 10px var(--danger); }

        /* CONTROL PANEL */
        .control-panel { padding: 20px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; background: var(--panel); border-bottom: 1px solid #333; }
        
        .input-group { display: flex; flex-direction: column; gap: 5px; }
        label { font-size: 10px; color: #666; letter-spacing: 1px; }
        input { background: #000; border: 1px solid #333; color: #fff; padding: 10px; font-family: inherit; font-size: 14px; outline: none; transition: 0.3s; }
        input:focus { border-color: var(--neon); box-shadow: 0 0 5px rgba(0, 243, 255, 0.3); }

        .btn-group { display: flex; gap: 10px; align-items: flex-end; }
        button { flex: 1; padding: 12px; font-family: inherit; font-weight: bold; cursor: pointer; border: none; font-size: 14px; color: #000; transition: 0.2s; clip-path: polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px); }
        
        .btn-start { background: var(--term); box-shadow: 0 0 5px var(--term); }
        .btn-start:hover { background: #00ff55; box-shadow: 0 0 15px var(--term); transform: translateY(-2px); }
        
        .btn-stop { background: var(--danger); box-shadow: 0 0 5px var(--danger); }
        .btn-stop:hover { background: #ff3366; box-shadow: 0 0 15px var(--danger); transform: translateY(-2px); }

        /* TERMINAL */
        .terminal-container { flex: 1; padding: 10px; overflow: hidden; display: flex; flex-direction: column; }
        .term-header { font-size: 11px; color: #555; margin-bottom: 5px; border-bottom: 1px solid #222; padding-bottom: 5px; display: flex; justify-content: space-between; }
        .terminal-body { flex: 1; overflow-y: scroll; padding: 10px; background: #000; border: 1px solid #222; box-shadow: inset 0 0 20px rgba(0,0,0,0.8); }
        
        .log-line { margin-bottom: 6px; font-size: 12px; border-left: 2px solid transparent; padding-left: 8px; line-height: 1.4; }
        .sys { color: #888; border-color: #555; }
        .in { color: var(--term); border-color: var(--term); }
        .out { color: var(--neon); border-color: var(--neon); }
        .err { color: var(--danger); border-color: var(--danger); }
        .game { color: #ffd700; border-color: #ffd700; background: rgba(255, 215, 0, 0.05); }

        /* SCROLLBAR */
        ::-webkit-scrollbar { width: 5px; }
        ::-webkit-scrollbar-thumb { background: #333; }
        ::-webkit-scrollbar-track { background: #000; }
    </style>
</head>
<body>
    <div class="header">
        <div class="title">TITAN OS // SERVER_NODE</div>
        <div id="status" class="status-badge offline">OFFLINE</div>
    </div>

    <div class="control-panel">
        <div class="input-group">
            <label>OPERATOR ID</label>
            <input type="text" id="u" placeholder="Username" value="">
        </div>
        <div class="input-group">
            <label>ACCESS KEY</label>
            <input type="password" id="p" placeholder="Password" value="">
        </div>
        <div class="input-group">
            <label>TARGET SECTOR</label>
            <input type="text" id="r" placeholder="Room Name" value="">
        </div>
        <div class="btn-group">
            <button class="btn-start" onclick="startBot()">INITIALIZE</button>
            <button class="btn-stop" onclick="stopBot()">SHUTDOWN</button>
        </div>
    </div>

    <div class="terminal-container">
        <div class="term-header">
            <span>LIVE_EVENT_LOGS [GAME_MODE: 4_CHIPS_WIN]</span>
            <span onclick="document.getElementById('logs').innerHTML=''" style="cursor:pointer;color:#555">[CLEAR]</span>
        </div>
        <div id="logs" class="terminal-body">
            <div class="log-line sys">System Standby. Enter credentials to initialize 24/7 Bot.</div>
        </div>
    </div>

    <script>
        function startBot() {
            const u = document.getElementById('u').value;
            const p = document.getElementById('p').value;
            const r = document.getElementById('r').value;
            if(!u || !p || !r) return alert("CREDENTIALS MISSING");
            
            fetch('/connect', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({u, p, r})
            }).then(res => res.json()).then(data => {
                console.log(data.status);
            });
        }

        function stopBot() {
            if(!confirm("CONFIRM EMERGENCY SHUTDOWN?")) return;
            fetch('/disconnect', {method: 'POST'}).then(res => res.json());
        }

        let isAutoScroll = true;
        const logContainer = document.getElementById('logs');
        
        logContainer.addEventListener('scroll', () => {
            if (logContainer.scrollTop + logContainer.clientHeight >= logContainer.scrollHeight - 10) {
                isAutoScroll = true;
            } else {
                isAutoScroll = false;
            }
        });

        setInterval(() => {
            fetch('/logs').then(res => res.json()).then(data => {
                // Update Status Badge
                const badge = document.getElementById('status');
                if(data.connected) {
                    badge.innerText = "ONLINE [ACTIVE]";
                    badge.className = "status-badge online";
                } else {
                    badge.innerText = "OFFLINE [STANDBY]";
                    badge.className = "status-badge offline";
                }

                // Render Logs
                const container = document.getElementById('logs');
                container.innerHTML = "";
                data.logs.forEach(l => {
                    const div = document.createElement('div');
                    div.className = `log-line ${l.type}`;
                    div.innerHTML = `<b>[${l.time}]</b> ${l.msg}`;
                    container.appendChild(div);
                });

                if(isAutoScroll) container.scrollTop = container.scrollHeight;
            });
        }, 2000);
    </script>
</body>
</html>
"""

if __name__ == '__main__':
    port = int(os.environ.get("PORT", 5000))
    app.run(host='0.0.0.0', port=port)